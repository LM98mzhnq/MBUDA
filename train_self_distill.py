...
def train(cfg, local_rank, distributed):
    ...
    optimizer_cls1 = torch.optim.SGD(classifier1.parameters(), lr=cfg.SOLVER.BASE_LR * 10, momentum=cfg.SOLVER.MOMENTUM,
    ...
    if cfg.resume:
        ...
        classifier_weights1 = checkpoint['classifier'] if distributed else strip_prefix_if_present(
    ...
    src_train_data1 = build_dataset(cfg, mode='train', is_source=True,index=1)
    ...
    for i, ((src_input, src_label, _),(src_input1, src_label1, _)) in enumerate(zip(train_loader,train_loader1)):
        ...
        src_input1 = src_input1.cuda(non_blocking=True)
        src_label1 = src_label1.cuda(non_blocking=True).long()
        size1 = src_label1.shape[-2:]


        pred = classifier(feature_extractor(src_input), size)
        loss0 = criterion(pred, src_label)
        pred1 = classifier1(feature_extractor(src_input1), size1)
        loss1 = criterion(pred1, src_label1)
        loss = loss0 +loss1
        loss.backward()
        ...
        output_dir = '/data/pth/lm-pth/MTDA1'
